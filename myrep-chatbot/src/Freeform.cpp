#include "../../myrep-chatbot/src/headers/Freeform.h"
#include <regex>
#include <string>
#include <fstream>
#include <iostream>
...
void Freeform::freeForm(std::string file)
{
    ...
    while (series >> words)
    {
        ...
        if (words == "name" || words == "contact" || words == "address" || words == "phone" || words == "message" || words == "party" || words == "live")
        {
            holder = "For ";
            holder += words;
            messages = " information, you should go look at Contact information, by parts or whole. We will now send you there.";
            holder += messages;
            std::cout << holder << endl;
            messagesthree = cpumessage + holder;
            r.noteOutput(messagesthree, file);
            fail = false;
            Contact w;
            w.getPart(file);
        }
        ...
        else if (words == "district" || words == "support"){
            messages = "We support district 59 for Rep Alexander Terry.";
            std::cout << messages << endl;
            r.noteOutput(messages, file);
            moving(file);
        }
        else if (words == "whole" || words == "entire" || words == "all" || words == "everything")
        {
            holder = "For the";
            holder += words;
            messages = " thing, you can select the print whole document option after going to Distinguishing.";
            holder += messages;
            std::cout << holder << endl;
            messagesthree = cpumessage + holder;
            r.noteOutput(messagesthree, file);
            fail = false;
            x.printWholeFile(file);
        }
